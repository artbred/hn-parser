...
func main() {
	var (
		...
		stopAtIDFlag   = flag.Int("stop-at-id", 0, "Stop processing when this ID is reached (exclusive)")
		...
		minScore       = flag.Int("min-score", 1, "Minimum score filter (0 = no filter)")
	...
	} else if *stopAtIDFlag > 0 {
		stopAtID = *stopAtIDFlag
		log.Printf("Incremental update mode: stopping at ID %d", stopAtID)
	}
